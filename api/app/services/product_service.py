...
from app.models.product import Product, ProductPydantic, Product_Pydantic_List
...
class ProductService:
    ...
    async def get_all_products(
        ...
        if pagination:
            ...
            filter_data = pagination.model_dump(exclude_none=True)
            filter_mappings = {
    ...
    async def get_products_by_category(self, category_name: str) -> List[ProductOut]:
        products = (
            await Product.filter(category__name=category_name)
            .order_by("-created_at")
    ...
    async def search_products(self, query: str) -> List[ProductOut]:
        ...
        products = (
            await Product.filter(title__icontains=query)
